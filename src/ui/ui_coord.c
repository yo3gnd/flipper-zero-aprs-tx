...
static char* coord_buf(FlipperHamApp* app) {
    if(app->text_mode == 8) return app->p_lon_edit;
...
static void coord_keypad_frame(Canvas* canvas) {
    ...
    for(i = 0; i <= 3; i++)
        canvas_draw_line(
            canvas,
            COORD_KEYS_X + i * COORD_KEY_W,
            COORD_KEYS_Y,
            COORD_KEYS_X + i * COORD_KEY_W,
            COORD_KEYS_Y + 4 * COORD_KEY_W);
    for(i = 0; i <= 4; i++)
        canvas_draw_line(
            canvas,
            COORD_KEYS_X,
            COORD_KEYS_Y + i * COORD_KEY_W,
            COORD_KEYS_X + 3 * COORD_KEY_W,
            COORD_KEYS_Y + i * COORD_KEY_W);
}
...
static bool coord_digit_ok(FlipperHamApp* app, char c) {
    ...
    for(i = 0; s[i]; i++)
        if(s[i] == '.') return coord_frac_digits(s) < COORD_FRAC_N;
